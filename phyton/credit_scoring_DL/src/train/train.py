...
class CreditScoringModelTraining:
    def __init__(self, config_path: Path) -> None:
        ...
        # Get project root (credit_scoring_DL directory) - resolve config path first, then go up 3 levels from config/training/file.yaml
        config_path_resolved = config_path.resolve()
        self.project_root = config_path_resolved.parent.parent.parent
        
        # paths - resolve dataset path relative to project root
        dataset_path_str = self.params['data_source']['data_path']['dataset_path']
        if Path(dataset_path_str).is_absolute():
            self.dataset_path = Path(dataset_path_str)
        else:
            # Resolve relative to project root
            self.dataset_path = (self.project_root / dataset_path_str).resolve()
...
python src/train/train.py --config config/training/credit_scoring-training_config-german_credit_risk_v100.yaml
python src/train/train.py --config config/training/credit_scoring-training_config-german_credit_risk_v110.yaml
python src/train/train.py --config config/training/credit_scoring-training_config-german_credit_risk_v120.yaml
python src/train/train.py --config config/training/credit_scoring-training_config-german_credit_risk_v130.yaml
"""

# Resumen del Flujo de Ejecución (train())
# Cuando ejecutas python train.py:

# Lectura: Lee el YAML de configuración.

# Preparación: Carga el CSV, limpia los datos y entrena el Preprocesador.

# Inicialización: Crea la arquitectura de red neuronal (CreditScoringModel) según el YAML (ej. 2 capas de 64 neuronas).

# Bucle: Empieza a iterar (Epochs).

# Entrena con un lote.

# Evalúa con validación.

# Si mejora, guarda. Si no mejora por un tiempo, para.

# Reporte: Genera gráficos de Curva ROC, Matriz de Confusión y los sube a MLflow.

# Empaquetado: Guarda el modelo final y el preprocesador listos para usar.