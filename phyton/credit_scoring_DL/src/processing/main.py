...
class CreditDataPreprocessor:
    ...
    def fit_preprocessor(self, df: pd.DataFrame) -> ColumnTransformer:
        ...
        # devuelve una matriz ya no es un dataframe de  pandas
  ...
  #def process_data(...) -> Tuple[np.ndarray, pd.Series]:
    def process_data(self, df: pd.DataFrame, preprocessor: ColumnTransformer) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        df_copy = df.copy() #Principio: Esto respeta el principio de inmutabilidad.recibe datos, devuelve un resultado nuevo, y no rompa nada de lo que recibió.
        df_copy[self.target_feature] = df_copy[self.target_feature].map({'bad': 0, 'good': 1}) #bad o good se definen segun que es para nosotros la clase positiva y la clase negativa
